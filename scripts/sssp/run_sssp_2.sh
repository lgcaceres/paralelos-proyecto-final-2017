...
#for i in {1..80}
do
  spray_w[$i]=`$bin -i $input -o out2.txt -n $i -l -w -m 2000000 | grep $var | grep '(?<= )[0-9]+\.?[0-9]*' -Po`
  lotan_shavit_w[$i]=`$bin -i $input -o out2.txt -n $i -p -w -m 2000000 | grep $var | grep '(?<= )[0-9]+\.?[0-9]*' -Po`
  linden_w[$i]=`$bin -i $input -o out2.txt -n $i -L -w -m 2000000 | grep $var | grep '(?<= )[0-9]+\.?[0-9]+' -Po`

  echo $i,${spray_w[$i]},${lotan_shavit_w[$i]},${linden_w[$i]}
done
